...
- Mobile is intentionally list → editor rather than a compressed multi-column canvas. Filters and sources use bottom drawers above the 64 px bottom navigation. The mobile editor keeps one Save action in the top header while source and delete remain visible above bottom navigation; mobile has no bulk-clear entry in this approved design. Named actions are at least 44 × 44 and preserve visible focus treatment.
...
- Desktop and tablet clear confirm use the linked medium Dialog and explicitly limit the destructive action to Notes; documents and QA records are not deleted. Mobile acceptance proves the same clear API/tombstone invariant while keeping per-note delete as the only destructive Notes UI in the editor flow. The source-deleted tombstone remains non-destructive to the note itself.
