...
- Focused test coverage (36 tests total across four Task 5 test files; categories overlap):
  ...
  - Query/API: 10 cases cover exact keys/routes/FormData, active-only polling, single focus refetch, success invalidation, active-page server-summary authority, delete success/failure refresh, unmount cache removal, rejection of a late user-A write before user B mounts and rejection after B's replacement Query is active.
...
- Acceptance criteria:
  ...
  - [x] Fixed query keys cannot retain one authenticated user's document/import data after the page unmounts; both queries use zero garbage-collection time, and authoritative mutation batches write only when the exact Query instance captured at mutation start remains current and observed.
...
- Scope confirmation:
  ...
  - Final re-review correction commit `3b22198` changes only the Task 5-owned query implementation and focused query test.
...
- Independent-review corrections:
  - Both fixed-key authenticated queries now use `gcTime: 0`; the regression loads user A, unmounts, proves both cache entries are absent, then mounts user B without ever rendering A data.
  - The manual `window.focus` listener was removed. TanStack's visibility mechanism is the sole focus-refetch owner; a hidden-to-visible refetch followed by a later focus event performs only one GET.
  - Import task rows map parsing/chunking/embedding/persisting/committing, distinguish retry-wait, show pending cancellation and suppress duplicate cancel; active state wins the mixed-batch title.
  ...
  - Final re-review RED: after user A unmounted and its query disappeared, user B mounted a replacement observed Query before A's deferred submit resolved; A's result replaced B in cache and DOM. All four import mutations now capture Query identity in `onMutate`, and shared success handling requires identity equality plus an observer; query tests pass `10/10`, Task 5 focused tests `36/36` and the frontend suite `101/101`.
- Deviations: None.
- Residual risks:
