...
test("scoped search, source details, editable note and QA handoff use real services", async ({ appUrl, page, context }, testInfo) => {
  ...
  await expect(page.getByRole("button", { name: "检索文档" })).toBeDisabled();
  ...
  await page.getByLabel("想在资料中找到什么？").fill("检索增强学习");
  ...
  await page.goto(`${appUrl}/search`);
  await expect(page.getByLabel("想在资料中找到什么？")).toHaveValue("");
  ...
  await expect.poll(async () => (await (await page.request.get(`${appUrl}/api/v1/notes/${note.id}`)).json()).sources[0].deleted).toBe(true);
