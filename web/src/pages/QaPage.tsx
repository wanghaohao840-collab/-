...
import { newClientRequestId, useQaCapabilities, useQaConversation, useQaConversations, useQaDeletion, useQaJob, useQaMessages, useQaMutations } from "../features/qa/queries";
...
export function QaPage() {
  ...
  const busy = mutations.ask.isPending || currentMessages.some((item) => item.status === "pending");
  const queryError = conversations.error ?? conversation.error ?? messages.error;
  ...
  return <article className="qa-page">
    <header className="qa-header"><div><h1>智能问答</h1><p>{conversation.data ? `基于 ${conversation.data.documents.length} 篇文档` : "选择固定文档范围，开始可追溯问答"}</p></div><div className="qa-header__actions"><Button hierarchy="secondary" className="qa-conversation-trigger" onClick={(event) => { setConversationTrigger(event.currentTarget); setConversationOpen(true); }}>对话</Button>{selectedId ? <><Button hierarchy="secondary" onClick={() => mutations.summarize.mutate({ conversationId: selectedId, instruction: "", clientRequestId: newClientRequestId() }, { onSuccess: (value) => setSummaryJobId(value.job_id) })}>生成摘要</Button><Button hierarchy="ghost" onClick={() => setDeleteOpen(true)}>删除对话</Button></> : null}</div></header>
    ...
    <SummaryStatus job={job.data} onCancel={() => summaryJobId && mutations.cancel.mutate(summaryJobId)} />
    <div className="qa-workspace">
      {conversations.isPending ? <aside className="qa-conversations" role="status">正在加载对话…</aside> : <ConversationList items={items} selectedId={selectedId} onSelect={selectConversation} onNew={() => setCreateOpen(true)} />}
    ...
    {conversationOpen ? <QaDrawer title="选择对话" className="qa-overlay__panel--conversations" returnFocusTo={conversationTrigger} onClose={() => setConversationOpen(false)}><ConversationList items={items} selectedId={selectedId} onSelect={selectConversation} onNew={() => { setConversationOpen(false); setCreateOpen(true); }} /></QaDrawer> : null}
    {sourceOpen ? <QaDrawer title="引用来源" className="qa-overlay__panel--sources" returnFocusTo={sourceTrigger} onClose={() => setSourceOpen(false)}><SourcePanel sources={sources} /></QaDrawer> : null}
