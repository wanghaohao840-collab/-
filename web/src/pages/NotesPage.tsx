...
export function NotesPage() {
  ...
  useEffect(() => { if (!dirty) return; const previous = `${window.location.pathname}${window.location.search}${window.location.hash}`; const previousIndex = window.history.state?.idx; const onPopState = () => { if (popstateRestoring.current) { popstateRestoring.current = false; return; } if (window.confirm("当前笔记有未保存更改，确定离开吗？")) { setDirty(false); return; } const currentIndex = window.history.state?.idx; const delta = historyRestoreDelta(previousIndex, currentIndex); if (delta) { popstateRestoring.current = true; window.history.go(delta); } else navigate(previous, { replace: true }); }; window.addEventListener("popstate", onPopState); return () => window.removeEventListener("popstate", onPopState); }, [dirty, navigate]);
