...
export function NotesPage() {
  ...
  function resetFilters() { if (!confirmDiscardIfDirty()) return; const next = new URLSearchParams(params); for (const key of ["query", "tags", "source_kind", "qa_message_id", "citation_id"]) next.delete(key); setParams(next); }
  function openSearch() { if (!confirmDiscardIfDirty()) return false; navigate("/search"); return true; }
  ...
  if (notes.isPending) return <section className="notes-state" role="status"><h1>学习笔记</h1><p>正在加载笔记库…</p></section>;
