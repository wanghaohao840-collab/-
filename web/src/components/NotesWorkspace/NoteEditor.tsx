...
export function NoteEditor({ draft, dirty, saving, disabled = false, error, onChange, onSave, onDelete, onOpenSources, onBack }: Props) {
  ...
  return <section className="notes-editor" aria-label="笔记编辑器">
    <header className="notes-editor__heading">{onBack ? <button type="button" className="notes-editor__back" aria-label="返回笔记列表" onClick={onBack}>←</button> : null}<div><h2>{draft.concept || "新建笔记"}</h2><p>{dirty ? "有未保存更改" : "已保存"}</p></div></header>
    ...
    <footer className="notes-editor__footer"><p>{dirty ? "尚未保存" : "上次保存"}</p><div className="notes-editor__actions">{onOpenSources ? <Button hierarchy="secondary" aria-label="来源" onClick={onOpenSources}>查看来源</Button> : null}<Button disabled={!dirty || saving || disabled} loading={saving} onClick={onSave}>保存笔记</Button>{onDelete ? <Button hierarchy="danger" disabled={saving} onClick={(event) => onDelete(event.currentTarget)}>删除笔记</Button> : null}</div></footer>
  </section>;
}
