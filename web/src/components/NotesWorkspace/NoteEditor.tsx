...
type Props = { draft: EditorDraft; dirty: boolean; saving: boolean; error?: string; onChange: (draft: EditorDraft) => void; onSave: () => void; onDelete?: () => void; onPreviewError?: (error: string) => void };

export function NoteEditor({ draft, dirty, saving, error, onChange, onSave, onDelete }: Props) {
  const [mode, setMode] = useState<"edit" | "preview">("edit");
  return <section className="notes-editor" aria-label="笔记编辑器">
    <header className="notes-editor__heading"><div><h2>{draft.concept || "新建笔记"}</h2><p>{dirty ? "有未保存更改" : "已保存"}</p></div><div className="notes-editor__actions"><Button hierarchy="secondary" disabled={!dirty || saving} loading={saving} onClick={onSave}>保存笔记</Button>{onDelete ? <Button hierarchy="danger" disabled={saving} onClick={onDelete}>删除笔记</Button> : null}</div></header>
